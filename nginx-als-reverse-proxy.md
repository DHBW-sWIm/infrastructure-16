...
In dem Ordner *sites-enabled* sollten die Konfigurationsdateien [`activiti.conf`](docker-compose/Nginx/activiti.conf) und [`moodle.conf`](docker-compose/Nginx/moodle.conf) liegen. Ohne diese Konfiguration sind die beiden Dienste von außerhalb nicht erreichbar.
